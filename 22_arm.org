...
Armv8-A の機械語プログラムの実行に QEMU エミュレータを使っているため、
通常の GDB の使い方とは異なり、
QEMU の GDB サーバを起動して、
...
そのためここでは GAS の AArch64 に固有の機能を中心に説明していきます。
...
ARM アーキテクチャは RISC です。
同じ RISC である AVR アーキテクチャと同じように、
Armv8-A のアドレス指定モードの種類はそれほど多くありません。
このため、
レジスタ名の先頭にパーセント (~%~) などを付けなくても、
GAS は、
ある文字列がレジスタ名なのかラベルなのかを判別することができます ([[sec:avr/gas]] 節)。

*** 再配置指定子 (relocation specifier)

Armv8-A の 64 ビットモードでは、
*すべての命令語長は 32 ビット* です。
IA-32 アーキテクチャの命令語は可変長であり、
AVR アーキテクチャは大半の命令語長が 16 ビット (一部の命令語長が 32 ビット) でした。

したがって、
Armv8-A では即値の扱いに制限があります。
命令語長が 32 ビットなので、
...
Armv8-A の AArch64 は 64 ビットモードなので、
レジスタの大きさも、
...
まず、
Armv8-A の 64 ビットモード (AArch64) における汎用レジスタを説明します。

Armv8-A には汎用レジスタが 31 個 (レジスタ X0〜X30) あります (図 [[fig:arm/regs]])。
AArch64 は 64 ビットモードであり、
当然、
レジスタの大きさも 64 ビットです。
Armv8-A は RISC なので、
...
#+begin_note
「Armv8-A には 31 個の汎用レジスタがある」と聞いて、
「え?
31 個?
32 個の間違いじゃないの?」
と思ったかもしれません。
...
実は、
Armv8-A には CMP 命令という専用の命令は存在せず、
...
Armv8-A の 64 ビットモード (AArch64) の命令セット (A64) には多数の命令があります。
...
- 汎用データ転送命令が多数ある。IA-32 アーキテクチャでは「何でもかんで
  も MOV 命令」で済ませていたが、Armv8-A ではデータの型や用途ごとに多
...
*プリ・インデックス (pre-index)* はオフセット付きのアドレス指定ですが、
メモリロケーションの計算の *前* に、
ベースレジスタに即値が加算されます。
これも例を示します。
#+begin_src asm-a64
...
#+end_src
出所オペランドの末尾の ~!~ が「プリ・インデックス」であることを表しています。
この 1 命令だけで、
...
*ポスト・インデックス (post-index)* とは、
上のプリ・インデックスと対になるアドレス指定です。
...
以下では、
COMET II や AVR アーキテクチャの命令の説明と同じような形式で Armv8-A の命令を説明します。
以下の表では、
これまでと同じように、
...
ベースレジスタとオフセット、
プリ・インデックス、
ポスト・インデックスによって決定されるメモリロケーションを意味します。
また、
...
- 末尾に ~!~ が付けば、それはプリ・インデックス
- 角括弧 (~[~ と ~]~) の後に即値があれば、それはポスト・インデックス
...
#+begin_src asm-a64
	...
	:
	.bss				// .bss セクションの開始
        .set    .LANCHOR0,. + 0		// .LANCHOR0 = 現在のアドレス
i:
        .zero   4
#+end_src
...
プリ・インデックスやポスト・インデックスのアドレス指定モードを利用することで、
スタック操作を実現できます。
...
|   1111 | NV           | 不明 (Not Valid?)     | 常に成立              | 常に成立            |
...
です。したがって、
\begin{align}
  -134 217 728 = -128 M \le \Delta \le 134217724 < 128M
\end{align}
であることがわかります。 
#+end_note
...
特に重要な命令は、
実行時間が短くなるように短い命令語が割り当てられ、
なおかつ機械語のプログラムをダンプした時に認識できるように単純なビットパターンの命令語が割り当てられるからです。
...
これまで、
Armv8-A の 64 ビットモード命令セット (A64) における主要な 18 個の命令を説明しました。
...
#+begin_src raw
-fno-pic -fomit-frame-pointer -O2 -fno-optimize-sibling-calls -fno-inline-functions -fno-inline-small-functions -fno-indirect-inl ining -fno-schedule-insns -fno-schedule-insns2
#+end_src
...
これらの命令のオペランドにプリ・インデックスやポスト・インデックスを使用して、
スタックの操作を実現します。
...
また、
配列 ~a~ は ~int~ 型の配列であり、
Armv8-A の 64 ビットモードにおける ~int~ の大きさは 32 ビットです。
したがって、
...
ただし、
Armv8-A では char 型を利用するとかってアセンブリ言語プログラムが複雑になるため、
