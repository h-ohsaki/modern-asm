...
SIMD も、
SISD と同じように、
一度に実行する命令は 1 つだけ (Single Instruction stream) ですが、
...
512 ビットのベクトルデータに対して整数演算も浮動小数点演算も可能になりました。
...
XMM レジスタによる浮動小数点演算の制御や、
浮動小数点演算の結果の取得に用いられるレジスタです。
...
それは、
「SSE2 の登場によって、
IA-32 アーキテクチャにおける浮動小数点演算の主役が x87 FPU から SSE2 に交代になった」からです。

前述のように、
IA-32 アーキテクチャの 64 ビット拡張に対応した CPU は、
必ず SSE2 に対応しています。
また最近の IA-32 アーキテクチャの CPU では、
x87 FPU による浮動小数点演算よりも、
SIMD 命令を用いた浮動小数点演算のほうが高速に動作するようです。
そのため、
64 ビットモード向けの C コンパイラは、
浮動小数点演算に x87 FPU 命令を使用せず、
代わりに SIMD 命令を使用します。
例えば、
対象とするコンピュータが 64 ビットの x86-64 である GCC は、
浮動小数点演算には x87 FPU 命令を使用せず、
SIMD 命令のみを使用します。
...
レジスタ MM0 の各要素と比較しています。

それぞれの値が等しければ、
レジスタ MM1 中の対応する要素の値を 0xff にし、
そうでなければ 0x00 にします。
例えば、
レジスタ MM0 の値が 0x0807 0605 0403 0201 であり、
レジスタ MM1 の値が 0x0800 0000 0000 0001 とします。
このときに、
\inst{pcmpeqb %mm0, %mm1} を実行すると、
最上位バイトと最下位バイトの要素が一致しているため、
レジスタ MM1 の値は 0xff00 0000 0000 00ff となります。
...
このため、
XMM レジスタには点の座標 ($x$, $y$) のデータがちょうど収まります。
