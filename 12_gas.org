...
これから例えば、
- ~_IO_printf~ 、 ~__printf~ 、 ~printf~ 、 ~__x86.get_pc_thunk.ax~ の
  4 つのシンボルは、 ~printf.o~ 中の .text セクション (プログラム) 中
  で定義されたシンボルである
- ~_GLOBAL_OFFSET_TABLE_~ 、 ~stdout~ 、 ~vfprintf~ というシンボルは
...
#+begin_src sh
$ ls -l add.o
-rw-r--r-- 1 asm asm 484 Feb 18 18:04 add.o
$ file add.o
...
#+begin_src asm
	movl    i, %eax		# ここはコメントです
        ...
        movl    %eax, i
#+end_src
*コメント開始文字* (~#~ や ~;~ など) から行末までがコメントになります。
面白いことに、
...
~L~ や ~.L~ から始まるシンボルは *局所シンボル (local symbol)* です。
局所シンボルはアセンブラがオブジェクトコードの生成に使用しますが、
オブジェクトファイル中には記録されません。
局所シンボルに何が使われるかも対象となるコンピュータによって異なります。
...
インテル構文で書かれた既存のアセンブリ言語プログラムを GAS に移植するような場合にはインテル構文が便利でしょう。
...
#+end_src
のように書くとデータサイズが判定できないためエラーになります (アセンブルに失敗します)。
...
何らかの計算結果によって定まる番地を「間接的に」指定する方法です。
...
ただし、
GAS バージョン 2.31.1 では 120 種類以上の疑似命令があります。
...
~.L~ から始まるラベルは ELF 形式のファイルで利用される (外部から参照されない) ローカルラベルです。
