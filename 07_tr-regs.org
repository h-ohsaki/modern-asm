...
これから、
現在のプログラムカウンタの値が 0x5655 6189 番地であること、
...
正確なコマンド名としては *stepi* (1 命令を実行する) ですが、
ここでは省略形の si を用いています。
...
また、
機械語の単純な命令の組み合わせてによって計算が実現されているのかがわかります。
